...
function GFBucket() {
    ...
    const Submit = (e) =>{
        ...
        axios.post("/bucket/single", formData, config)
        .then((res)=> {
            setProgress(0)
    ...
    const MuSubmit = (e) =>{
        ...
        axios.post("/bucket/multiple", formData, config)
        .then((res)=> {
            setmProgress(0)
    ...
    <>
        <div className="col-md-6 m-auto mb-2">
            <h3 className="text-center display-4 my-4"> GF Bucket </h3>
        </div>
        ...
        {
            pics && 
            names.map((name, i)=>(<React.Fragment key={i}> <Img Src={`/bucket/image/${name}`} /> </React.Fragment>)) 
        }
